...
class AnnRegression(AnnBase):
    ...
    def get_model(self):
        ...
        if self.number_of_hidden_layers > 0:
           model.add(Dense(self._number_of_neurons_in_first_hidden_layer, input_shape=(1, 27), kernel_initializer=self.kernel_initializer, activation=self.activation_function))
    ...
    def compile_fit_predict_with_current_model(self, testX):
        self.get_model_from_path(MODEL_NAME)
